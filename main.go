...
func ExportCSV(sheet *xlsx.Sheet, field_list []FieldInfo, filename string) {
	...
	csv_file := path.Join(path.Dir(filename), sheet.Name+".csv")
	if err := ioutil.WriteFile(csv_file, []byte(buffer), 0644); err == nil {
		fmt.Println("Exported", path.Base(csv_file), count)
	} else {
		fmt.Println(err)
	}
}
...
func ExportSQLFile(filename string) {
	...
	if count > 0 {
		...
		sql_file := filename[0:len(filename)-len(ext)] + ".sql"
		if err := ioutil.WriteFile(sql_file, []byte(buffer), 0644); err == nil {
			fmt.Println("Exported", path.Base(sql_file), count)
		} else {
			fmt.Println(err)
		}
	}
